...
export function RegistryTab() {
  ...
  const objectName = permanentData['Объект строительства'] || 
                     permanentData['Объект_строительства'] || 
                     permanentData['object_name'] || 
                     permanentData['Наименование объекта'] || 
                     permanentData['object'] || 
                     'Не указан';
  ...
  const handleGeneratePDF = async () => {
    ...
    const exportSession = {
      ...
      permanentData: storeState.permanentData,
      templates: storeState.templates,
      materials: storeState.materials,
      appendices: storeState.appendices,
      aosrActs: storeState.aosrActs,
      otherActs: storeState.otherActs,
      registry: storeState.registry,
      dateStart: storeState.dateStart,
      dateEnd: storeState.dateEnd,
      spList: storeState.spList,
    };

    try {
      const pdfBytes = await generateFullPackagePDF(exportSession);
      const pdfArray = new Uint8Array(pdfBytes);
      const blob = new Blob([pdfArray.buffer], { type: 'application/pdf' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${exportSession.name}_комплект.pdf`;
      a.click();
      URL.revokeObjectURL(url);
      toast.success('Комплект документации сформирован');
    } catch (e) {
      console.error('PDF Generate Error:', e);
      toast.error(`Ошибка формирования PDF: ${e instanceof Error ? e.message : String(e)}`);
    }
  };
  ...
  const handleExportDOCX = async () => {
    ...
    try {
      await exportDOCX(exportSession, exportWithAppendices);
  ...
  const handleExportExcel = () => {
    ...
    const data = registry.map((entry) => ({
      '№ п/п': entry.order,
      'Статус': getStatusLabel(entry.status),
      'Наименование документа': entry.documentName,
      '№ и дата': entry.docNumber,
      'Организация': entry.organization,
      'Листов': entry.pageCount,
      'Лист по списку': entry.pageInList,
    }));
    ...
    const safeName = objectName.replace(/[^a-zа-яё0-9]/gi, '_');
    XLSX.writeFile(workbook, `${safeName}_Реестр.xlsx`);
    toast.success('Реестр сохранен в Excel');
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="export-appendices" className="text-sm text-gray-600 mr-2">
              С приложениями
            </label>
            <Button
            ...
            <tbody>
              ...
              {registry.map((entry, index) => (
                ...
                >
                  ...
                  <td className="px-3 py-2">
                    <Textarea
                      id={`doc-name-${entry.id}`}
                      name={`doc-name-${entry.id}`}
                  ...
                  <td className="px-3 py-2">
                    <Input
                      id={`list-pages-${entry.id}`}
                      name={`list-pages-${entry.id}`}
