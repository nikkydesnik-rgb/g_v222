                ...
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <div className="flex-1 mr-4">
                      <Input
                        value={act.actName ?? template.name}
                        onChange={(e) => updateOtherAct(act.id, { actName: e.target.value })}
                        placeholder="Название акта (для реестра)"
                        className="h-8 font-semibold text-base border-transparent hover:border-gray-300 focus:border-blue-500 bg-transparent px-2 -ml-2"
                      />
                    </div>
                  ...
                  <div className="flex items-center space-x-2 px-1">
                    <Checkbox
                      id={`title-page-${act.id}`}
                      checked={!!act.isTitlePage}
                      onCheckedChange={(checked) => 
                        updateOtherAct(act.id, { isTitlePage: checked === true })
                      }
                    />
                    <label
                      htmlFor={`title-page-${act.id}`}
                      className="text-sm font-medium leading-none select-none text-muted-foreground"
                    >
                      Титульный лист (первый в сборке, скрыт из реестра)
                    </label>
                  </div>
                </div>
                <p className="text-xs text-gray-500 mt-2">{template.keys.length} ключей</p>
              </CardHeader>
