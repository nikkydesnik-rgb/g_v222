...
import { defaultOrganizations } from '@/data/organizations';
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      dateEnd: '',
      spList: [...defaultSPList],
      savedOrganizations: [...defaultOrganizations],
      includeMaterialDocs: true,

      // Organization Actions
      addSavedOrganization: (org) => set((state) => ({ savedOrganizations: [...state.savedOrganizations, org] })),
      removeSavedOrganization: (id) => set((state) => ({ savedOrganizations: state.savedOrganizations.filter(o => o.id !== id) })),
      updateSavedOrganization: (id, org) => set((state) => ({
        savedOrganizations: state.savedOrganizations.map(o => o.id === id ? { ...o, ...org } : o)
      })),
      ...
      renumberAOSRActs: () => {
        set((state) => ({
          aosrActs: state.aosrActs.map((act, index) => ({
            ...act,
            act_number: (index + 1).toString(),
          })),
        }));
      },
      ...
      autoPopulateRegistry: () => {
        ...
        state.aosrActs.forEach((act) => {
          entries.push({
            ...
            documentName: `Акт освидетельствования скрытых работ\n${act.work_name || ''}`,
            drawingNumber: '',
            docNumber: act.act_number?.toString() || '',
            organization: state.permanentData['Организация_строитель'] || state.permanentData['Организация - строитель'] || '',
            pageCount: '',
            pageInList: '',
            status: 'yellow',
            fileType: 'act',
        ...
        state.otherActs.forEach((act) => {
          if (act.isTitlePage) return; // Skip title pages from registry
          const template = state.templates.find((t) => t.id === act.templateId);
          entries.push({
            id: `other-${act.id}`,
            order: entries.length + 1,
            documentName: `Акт\n${act.actName || template?.name || 'Иной акт'}`,
            drawingNumber: '',
            docNumber: '',
            organization: state.permanentData['Организация_строитель'] || state.permanentData['Организация - строитель'] || '',
            pageCount: '',
