  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        console.warn('Rendering server endpoint /api/render-docx not found. Falling back to client-side rendering.');
      } else {
        console.warn(`Rendering server returned error ${response.status}. Falling back to client-side rendering.`);
      }
      return null;
    }
